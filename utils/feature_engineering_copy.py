"""Python module for feature engineering of IFTA trip data."""
...
def calculate_fuelling_cycle_features(df, max_gap_days=3):
    ...
    for idx, trip_date in zip(df.index, df['trip_date']):
        ...
        if min_gap_days[idx] > max_gap_days:
            continue  # No verifiable fuel anchor within the acceptable window
            # Downstream features will be NaN
...
def extract_temporal_trends(df):
    ...
    # This takes each 'trip_date' in df and looks it up in the daily_stats index
...
def calculate_odometer_gap(df):
    ...
    df_sorted['prev_end_odometer'] = (
        df_sorted.groupby('vin_or_truck_number')['end_odometer'].shift(1)
    )
    ...
    has_both = (df_sorted['start_odometer'].notna()
                & df_sorted['prev_end_odometer'].notna())
...
def engineer_ifta_features(df_ml_ready, window_days=3):
    """Master function for feature engineering."""
